...
/**
 * 频道列表浮层的分组过滤纯逻辑（ticket 05，浮层重构）。
 *
 * 标签序列 = 固定"收藏"（第一个）→ 固定"全部" → 各分组（orderIndex 升序）；
 * "收藏"与"全部"是固定虚拟标签，不落库（spec Implementation Decisions）。
 * 纯函数，可 JVM 单测。
 */
object ChannelListFilter {
    ...
    /** 标签序列：收藏 → 全部 → 各分组（已按 orderIndex 升序） */
    fun tabs(groups: List<String>): List<String> = listOf(TAB_FAVORITES) + listOf(TAB_ALL) + groups
