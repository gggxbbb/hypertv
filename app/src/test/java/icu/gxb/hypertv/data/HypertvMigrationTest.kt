...
/**
 * Room 迁移测试（v1 → v5）：手工按导出的 schema（app/schemas/N.json）建库并预置数据，
 * 再以当前版本打开执行对应 Migration。Room 打开时会对迁移后的 schema 与编译期期望
 * （5.json）做身份校验，迁移不正确会抛 IllegalStateException。
 */
...
class HypertvMigrationTest {
    ...
    fun `migrate 1 to 2 preserves data and adds epgUrl column`() = runTest {
        ...
        // 以当前版本打开：触发 MIGRATION_1_2 → 2_3 → 3_4 → 4_5，Room 内部校验迁移结果与期望 schema 一致
    ...
    fun `migrate 2 to 3 adds tables and column and migrates legacy source`() = runTest {
        ...
        // 以当前版本打开：触发 MIGRATION_2_3 + MIGRATION_3_4 + MIGRATION_4_5，Room 内部校验迁移结果与期望 schema（5.json）一致
        val db = Room.databaseBuilder(context, HypertvDatabase::class.java, dbName)
            .addMigrations(
                HypertvDatabase.MIGRATION_2_3,
                HypertvDatabase.MIGRATION_3_4,
    ...
    fun `migrate 3 to 4 adds epg_channels table and preserves data`() = runTest {
        ...
        // 以当前版本（5）打开：触发 MIGRATION_3_4 + MIGRATION_4_5，Room 内部校验迁移结果与期望 schema（5.json）一致
        val db = Room.databaseBuilder(context, HypertvDatabase::class.java, dbName)
            .addMigrations(HypertvDatabase.MIGRATION_3_4, HypertvDatabase.MIGRATION_4_5)
            .build()
        try {
    ...
    @Test
    fun `migrate 4 to 5 adds epgMatchSource column and preserves data`() = runTest {
        createV4Database()

        // 以当前版本（5）打开：触发 MIGRATION_4_5，Room 内部校验迁移结果与期望 schema（5.json）一致
        val db = Room.databaseBuilder(context, HypertvDatabase::class.java, dbName)
            .addMigrations(HypertvDatabase.MIGRATION_4_5)
            .build()
        try {
            // 既有数据完整保留
            assertEquals("源 1", db.playlistSourceDao().getById("src-1")?.name)
            ...
            assertEquals("http://x/epg.xml", db.epgSourceDao().getAllOnce().single().url)
            assertEquals(1, db.epgMatchRuleDao().getAllOnce().size)
            assertEquals("CCTV1", db.epgChannelDao().getAllOnce().single().displayName)
            // v5 新增列可空，存量行来源为 null（不追溯，后续刷新补写）
            assertNull(db.channelDao().getByIdOnce("ch-1")?.epgMatchSource)
        } finally {
            db.close()
        }

        // 迁移后的 epgMatchSource 可写可读（刷新/规则回写）
        val db2 = Room.databaseBuilder(context, HypertvDatabase::class.java, dbName).build()
        try {
            db2.channelDao().updateChannelEpgMatches(
                listOf(
                    icu.gxb.hypertv.data.entity.ChannelEpgMatchUpdate(
                        channelId = "ch-1",
                        epgId = "cctv1",
                        source = "level3",
                    ),
            ...
            assertEquals("level3", db2.channelDao().getByIdOnce("ch-1")?.epgMatchSource)
        } finally {
            db2.close()
        }
    }
    ...
    /** 按 4.json 的 createSql 建 v4 库（含 room_master_table 身份哈希）并预置数据。 */
    private fun createV4Database() {
        ...
        SQLiteDatabase.openOrCreateDatabase(dbFile, null).use { db ->
            ...
            db.execSQL(
                "INSERT OR REPLACE INTO room_master_table (id,identity_hash) " +
                    "VALUES(42, '9c825a9c7e3c5938dc51f9b688ef319b')",
            )
            db.version = 4
            ...
            db.execSQL(
                "INSERT INTO epg_channels (id, displayName, icon) VALUES ('1', 'CCTV1', 'http://x/1.png')",
            )
        }
    }
}
